...
- **Codex plugin packaging**: ARS now installs in one line via `/plugin marketplace add Imbad0202/academic-research-skills` + `/plugin install academic-research-skills`. The traditional `git clone + symlink to ~/.Codex/skills/` flow continues to work — both tracks are first-class. Repo gains four top-level directories: `.Codex-plugin/`, `commands/`, `agents/`, `hooks/`, plus a `skills/` symlink dir; existing 4 skill directories untouched.
...
## Version Info
- **Suite version**: 3.13.0 (display: v3.13.0-20260625; per plugin.json)
