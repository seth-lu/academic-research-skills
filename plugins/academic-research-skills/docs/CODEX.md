...
- `plugins/academic-research-skills/` is the marketplace-installable plugin package. Codex Git
  marketplaces enumerate plugins from `./plugins/<plugin-name>`, so this wrapper is intentionally
  self-contained.
...
If the marketplace was already added before this package layout existed, refresh it first:

```powershell
codex plugin marketplace upgrade academic-research-skills-codex
codex plugin add academic-research-skills@academic-research-skills-codex
```
